                     ...
                     {practiceFile ? (
                        <div className="animate-fade-in" style={{animationDelay: '100ms'}}>
                            ...
                            {activeMode === 'solution' ? (
                                ...
                                <fieldset className="mt-4"><legend className="text-base font-medium text-slate-900 dark:text-slate-200 mb-2">Lösungsmodus</legend><div className="grid grid-cols-2 gap-2 rounded-lg bg-slate-100 dark:bg-slate-800 p-1">{solutionModeOptions.map(o => <div key={o.id}><input type="radio" id={`sol-${o.id}`} name="solution-mode" value={o.id} checked={solutionMode === o.id} onChange={() => setSolutionMode(o.id as SolutionMode)} className="sr-only"/><label htmlFor={`sol-${o.id}`} className={`flex flex-col items-center justify-center cursor-pointer select-none rounded-md p-3 text-center text-sm font-medium transition-colors ${solutionMode === o.id ? `bg-white dark:bg-slate-700 ${theme['text-primary-600-dark-400']} dark:text-white shadow-sm` : 'hover:bg-white/60 dark:hover:bg-slate-700/60'}`}><o.icon className="h-6 w-6 mb-1" />{o.label}</label></div>)}</div></fieldset>
                            ) : (
                                // FIX: Cast o.id to SimulationModeType to satisfy the type of setSimulationMode.
                                <fieldset className="mt-4"><legend className="text-base font-medium text-slate-900 dark:text-slate-200 mb-2">Simulations-Typ</legend><div className="grid grid-cols-2 gap-2 rounded-lg bg-slate-100 dark:bg-slate-800 p-1">{simulationModeOptions.map(o => <div key={o.id}><input type="radio" id={`sim-${o.id}`} name="simulation-mode" value={o.id} checked={simulationMode === o.id} onChange={() => setSimulationMode(o.id as SimulationModeType)} className="sr-only"/><label htmlFor={`sim-${o.id}`} className={`flex flex-col items-center justify-center cursor-pointer select-none rounded-md p-3 text-center text-sm font-medium transition-colors ${simulationMode === o.id ? `bg-white dark:bg-slate-700 ${theme['text-primary-600-dark-400']} dark:text-white shadow-sm` : 'hover:bg-white/60 dark:hover:bg-slate-700/60'}`}><o.icon className="h-6 w-6 mb-1" />{o.label}</label></div>)}</div></fieldset>
                            )}
                        </div>
                    ) : (
                    ...
                    <div className="animate-fade-in mt-8" style={{animationDelay: '400ms'}}>
                        {practiceFile ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                 <button onClick={handleGenerateGuide} className={`w-full px-6 py-3 ${theme['bg-primary-600']} text-white font-bold text-lg rounded-lg shadow-lg ${theme['hover:bg-primary-700']} focus:outline-none focus:ring-2 ${theme['focus:ring-primary-400']} focus:ring-opacity-75 transition-all transform hover:scale-105 flex items-center justify-center`}><BookOpenIcon className="h-6 w-6 mr-2" /> Lern-Guide erstellen</button>
      ...
      <header className="py-4 px-4 sm:px-6 lg:px-8 border-b border-slate-200 dark:border-slate-800">
        <div className="flex items-center justify-between max-w-6xl mx-auto">
            <div className="flex items-center">
              <BrainCircuitIcon className={`h-8 w-8 ${theme['text-primary-500']}`} />
