...
import { ChevronDownIcon, SendIcon, SparklesIcon, BookOpenIcon, LightBulbIcon, ExclamationTriangleIcon, ListBulletIcon, ChatBubbleLeftRightIcon } from './icons';
  ...
  return (
    <div className="border-b border-slate-200 dark:border-slate-700">
      <h2 id={`accordion-header-${step.title.replace(/\s+/g, '-')}`}>
        <button
          type="button"
          className={`flex items-center justify-between w-full p-5 font-medium text-left text-slate-700 dark:text-slate-300 hover:bg-slate-100/50 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 ${theme['focus:ring-primary-300_dark-800']}`}
          onClick={onClick}
      ...
      <div id={`accordion-body-${step.title.replace(/\s+/g, '-')}`} className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[4000px]' : 'max-h-0'}`} role="region">
        <div className="p-6 space-y-8 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
          
          {/* Main Content Block */}
          <div>
            <h4 className="text-md font-semibold text-slate-700 dark:text-slate-300 mb-3 flex items-center">
                <BookOpenIcon className={`h-5 w-5 mr-2 ${theme['text-primary-500']}`} />
                Erklärung
            </h4>
            <div className="p-4 rounded-md bg-white dark:bg-slate-900/50">
                <MarkdownRenderer content={step.content} />
            </div>
          </div>
          
          {/* Suggested Follow-ups Block */}
              ...
              <div className="flex flex-wrap gap-2">
                {step.suggestedFollowUps.map((q, index) => (
                  <button
                    ...
                    disabled={isAsking}
                    className={`px-3 py-1.5 text-sm rounded-full transition-all duration-300 flex items-center ${activeButtonIndex === index ? 'bg-green-200 dark:bg-green-800 scale-105' : `bg-slate-200 dark:bg-slate-700/50 text-slate-700 dark:text-slate-200 hover:bg-slate-300/70 dark:hover:bg-slate-700`} focus:outline-none focus:ring-2 focus:ring-offset-2 ${theme['focus:ring-primary-500']} disabled:opacity-70 disabled:cursor-not-allowed`}
                  >
                    {q}
                  </button>
                ))}
          ...
          <div className="pt-6 border-t border-slate-200 dark:border-slate-700">
            ...
            <form onSubmit={handleFormSubmit} className="relative">
              <textarea value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Stelle hier deine Frage zum obigen Inhalt..." className={`w-full p-3 pr-12 border border-slate-300 dark:border-slate-600 rounded-lg focus:ring-2 ${theme['focus:ring-primary-500']} transition-colors bg-white dark:bg-slate-900/50`} rows={2} disabled={isAsking} />
...
const SolvedQuestionAccordionItem: React.FC<{ item: SolvedQuestion; isOpen: boolean; onClick: () => void; index: number; onAskFollowUp: (question: string) => Promise<void>; }> = ({ item, isOpen, onClick, index, onAskFollowUp }) => {
    ...
    return (
        ...
        <h2 id={`accordion-header-${item.title.replace(/\s+/g, '-')}`}>
            ...
            >
            <span className="text-lg text-left pr-4"><strong>{item.title}</strong> {item.questionText}</span>
            <ChevronDownIcon className={`w-6 h-6 shrink-0 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
            </button>
        </h2>
        <div id={`accordion-body-${item.title.replace(/\s+/g, '-')}`} className={`overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'max-h-[4000px]' : 'max-h-0'}`} role="region">
            <div className="p-6 space-y-6 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700">
                <div className="pb-4 border-b border-slate-200 dark:border-slate-700">
                    <h3 className={`text-md font-semibold text-slate-700 dark:text-slate-300 mb-2`}>Frage</h3>
                    <p className="text-slate-800 dark:text-slate-200">{item.questionText}</p>
                </div>
                <div>
...
export const GuideDisplay: React.FC<ResultDisplayProps> = React.memo(({ guide, solvedQuestions, onAskFollowUp, onAskFollowUpOnSolution, openStepIndex, onStepClick, onSolveNextBatch, isSolvingNextBatch, hasMoreQuestions, onGenerateNextBatch, isGeneratingNextBatch, hasMoreTopics }) => {
  const { theme } = useTheme();
  ...
  return (
    <div className="w-full max-w-4xl mx-auto bg-white dark:bg-slate-800/50 rounded-lg shadow-lg overflow-hidden">
        <div className={`p-5 border-b border-slate-200 dark:border-slate-700`}>
             <h2 className={`text-2xl font-bold ${theme['text-primary-600_dark-400']}`}>
                {isGuideMode ? 'Dein persönlicher Lern-Guide' : 'Gelöste Übungsaufgaben'}
            </h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                {isGuideMode ? 'Klicke auf die einzelnen Schritte, um die Erklärungen und interaktiven Elemente anzuzeigen.' : 'Klicke auf die Aufgaben, um die detaillierten Lösungen und Erklärungen zu sehen.'}
            </p>
        </div>
        ...
        {isGuideMode && hasMoreTopics && onGenerateNextBatch && (
