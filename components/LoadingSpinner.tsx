...
// FIX: Add an optional `message` prop to the component's props interface.
...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({ progress, message: customMessage }) => {
  ...
  useEffect(() => {
    // FIX: Use the custom message if it's provided, otherwise fall back to progress-based messages.
    if (customMessage) {
  ...
  return (
    <div className="flex flex-col items-center justify-center text-center p-8 w-full max-w-2xl mx-auto">
      ...
      <p className="mt-2 text-sm font-medium text-indigo-600 dark:text-indigo-400">{Math.round(progress)}%</p>
    </div>
  );
};
